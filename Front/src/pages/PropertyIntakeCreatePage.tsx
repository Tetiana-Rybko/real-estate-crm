...
import { createPropertyIntake } from "../app/propertyIntakes.api";
...
export default function PropertyIntakeCreatePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        setError(err.response?.data?.detail || "Не вдалося зберегти акт");
      } else {
        setError("Не вдалося зберегти акт");
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      <div>
        <h1 style={pageTitleStyle}>Новий акт прийому</h1>
        <p style={{ margin: "8px 0 0", color: "#7A5A68" }}>
          Заповніть дані по об'єкту для подальшого опрацювання в офісі
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: "grid", gap: 20 }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          ...
          >
            {loading ? "Збереження..." : "Зберегти акт"}
          </button>
        </div>
      </form>
    </div>
